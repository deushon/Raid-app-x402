...
3. Тело JSON: обязательно **`message`** (строка). Рекомендуется **`metadata`**: **`task_id`**, **`error_context`** (строка, может быть `""`), опционально **`situation_report`** (длинный UTF-8 текст для оператора/VR). Без **`metadata`** сервер всё равно примет запрос и заполнит стандартные поля пустыми строками.
4. **201** — новая заявка; **200** + **`duplicate: true`** — заявка уже открыта (можно не спамить). **400** — нет строкового **`message`**.
