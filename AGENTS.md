...
- Коммитить **`.env`**, реальные ключи в JSON-конфигах, **архивы снимков** репозитория (`*.zip` и т.п.), содержимое каталога **`private/`** (локальные черновики; публичная статика — в `public/`).
